...
class Sword extends \pocketmine\item\Sword implements Enchantable {
    ...
    public function getLoreForItem(): array {
        ...
        if($this->hasItemSkin()) {
            ...
            $lore[] = "";
            $lore[] = TextFormat::RESET . TextFormat::BLUE . "+" . $this->getAttackPoints() . " Attack Damage";
        }
        $lore[] = "";
    ...
    private const SWORD_IDS = [ItemIds::WOODEN_SWORD, ItemIds::GOLDEN_SWORD, ItemIds::STONE_SWORD, ItemIds::IRON_SWORD, ItemIds::DIAMOND_SWORD];

    public function hasItemSkin(): bool {
        return !in_array($this->getId(), self::SWORD_IDS);
    }
    ...
    public function copyInto(Item $item): Item {
        ...
        if($item instanceof Sword && !$item->hasItemSkin()) {
