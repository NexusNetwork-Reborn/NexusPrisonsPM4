...
class EliteContraband extends ContrabandRewards {

    /**
     * EliteContraband constructor.
     */
    public function __construct() {
        $rewards = [
            new Reward("Mystery Elite Enchant", function(?NexusPlayer $player): Item {
                $item = (new MysteryEnchantmentBook(Rarity::ELITE))->toItem()->setCount(3);
                if($player !== null) {
            ...
            new Reward("Charge Orb", function(?NexusPlayer $player): Item {
                $item = (new ChargeOrb(mt_rand(3, 5)))->toItem()->setCount(1);
                if($player !== null) {
                    $player->getInventory()->addItem($item);
                }
                return $item;
            ...
            new Reward("Charge Orb Slot", function(?NexusPlayer $player): Item {
                $item = (new ChargeOrbSlot())->toItem()->setCount(7);
                if($player !== null) {
                    $player->getInventory()->addItem($item);
                }
                return $item;
            ...
            new Reward("Ulterior Rank", function(?NexusPlayer $player): Item {
                $item = (new RankNote(Nexus::getInstance()->getPlayerManager()->getRankManager()->getRankByIdentifier(Rank::SUPREME)))->toItem()->setCount(1);
                if($player !== null) {
                    $player->getInventory()->addItem($item);
                }
                return $item;
            }, 10),
            new Reward("64k - 256k Energy", function(?NexusPlayer $player): Item {
                $item = (new Energy(64000 * mt_rand(1, 4)))->toItem()->setCount(1);
                if($player !== null) {
                    $player->addItem($item, true);
                }
                return $item;
            }, 10000),
            new Reward("$32,000 - $128,000", function(?NexusPlayer $player): Item {
                $item = (new MoneyNote(32000 * mt_rand(1, 4)))->toItem()->setCount(1);
                if($player !== null) {
                    $player->getInventory()->addItem($item);
                }
                return $item;
            ...
            new Reward("Black/White Scrolls", function(?NexusPlayer $player): Item {
                if(mt_rand(1, 2) === 1) {
                    ...
                    $item = (new BlackScroll(mt_rand(20, 50)))->toItem()->setCount(1);
                }
                if($player !== null) {
                    $player->getInventory()->addItem($item);
                }
            ...
            new Reward("Satchel", function(?NexusPlayer $player): Item {
                if(mt_rand(1, 3) === 1) {
                    $item = (new Satchel(ItemFactory::getInstance()->get(ItemIds::DYE, 4)))->toItem()->setCount(1);
                }
                else {
                    $item = (new Satchel(ItemFactory::getInstance()->get(ItemIds::LAPIS_ORE)))->toItem()->setCount(1);
                }
                if($player !== null) {
                    $player->getInventory()->addItem($item);
                }
            ...
            new Reward("Golden Weapons/Tools", function(?NexusPlayer $player): Item {
                $limit = 5 * mt_rand(2, 3);
                if(mt_rand(1, 2) === 1) {
                    if(mt_rand(1, 2) === 1) {
                        $item = ItemFactory::getInstance()->get(ItemIds::GOLDEN_SWORD, 0, 1);
