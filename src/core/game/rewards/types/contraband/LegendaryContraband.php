...
class LegendaryContraband extends ContrabandRewards {

    /**
     * LegendaryContraband constructor.
     */
    public function __construct() {
        $rewards = [
            new Reward("Mystery Legendary Enchant", function(?NexusPlayer $player): Item {
                $item = (new MysteryEnchantmentBook(Rarity::LEGENDARY))->toItem()->setCount(3);
                if($player !== null) {
            ...
            new Reward("Enchant Re-Rolls", function(?NexusPlayer $player): Item {
                $item = (new EnchantmentReroll())->toItem()->setCount(10);
                if($player !== null) {
                    $player->getInventory()->addItem($item);
                }
                return $item;
            ...
            new Reward("Charge Orb", function(?NexusPlayer $player): Item {
                $item = (new ChargeOrb(mt_rand(7, 9)))->toItem()->setCount(1);
                if($player !== null) {
                    $player->getInventory()->addItem($item);
                }
                return $item;
            ...
            new Reward("Charge Orb Slot", function(?NexusPlayer $player): Item {
                $item = (new ChargeOrbSlot())->toItem()->setCount(12);
                if($player !== null) {
                    $player->getInventory()->addItem($item);
                }
                return $item;
            }, 5000),
            new Reward("Martian Rank", function(?NexusPlayer $player): Item {
            ...
            new Reward("256k - 1024k Energy", function(?NexusPlayer $player): Item {
                $item = (new Energy(256000 * mt_rand(1, 4)))->toItem()->setCount(1);
                if($player !== null) {
                    $player->addItem($item, true);
                }
                return $item;
            }, 10000),
            new Reward("$128,000 - $512,000", function(?NexusPlayer $player): Item {
                $item = (new MoneyNote(128000 * mt_rand(1, 4)))->toItem()->setCount(1);
                if($player !== null) {
                    $player->getInventory()->addItem($item);
                }
                return $item;
            ...
            new Reward("Black/White Scrolls", function(?NexusPlayer $player): Item {
                if(mt_rand(1, 2) === 1) {
                    ...
                    $item = (new BlackScroll(mt_rand(50, 80)))->toItem()->setCount(1);
                }
                if($player !== null) {
                    $player->getInventory()->addItem($item);
                }
