...
class NeutralSubCommand extends SubCommand {

    /**
     * EnemySubCommand constructor.
     */
    public function __construct() {
        parent::__construct("neutral", "/gang neutral <gang>");
    ...
    public function execute(CommandSender $sender, string $commandLabel, array $args): void {
        ...
        if(!$senderGang->isEnemy($gang)) {
            $sender->sendMessage(Translation::getMessage("notEnemy"));
            return;
        }
        $senderGang->removeEnemy($gang);
        foreach($senderGang->getOnlineMembers() as $member) {
            if(!$member->isLoaded()) {
                continue;
            }
            $member->sendTranslatedMessage("neutral", [
