...
class ItemBaseEntity extends Entity implements AnimationEntity {
    ...
    protected function getInitialSizeInfo(): EntitySizeInfo {
        return new EntitySizeInfo(0.85, 0.001);
    }
     ...
     */
    public static function create(NexusPlayer $owner, Item $item, Position $position): ItemBaseEntity {
        $entity = new ItemBaseEntity(Location::fromObject($position, $position->getWorld()));
    ...
    protected function entityBaseTick(int $tickDiff = 1): bool {
        ...
        foreach ($this->getViewers() as $viewer){
            /** @var NexusPlayer $viewer */
            $this->sendSkin($viewer);
        }
        ...
        elseif($this->motion->x === 0 and $this->motion->z === 0) {
    ...
    protected function sendSpawnPacket(Player $player): void {
        ...
        if($entity->getUniqueId()->toString() !== $player->getUniqueId()->toString()) {
            return;
        }
    ...
    public function sendSkin(NexusPlayer $target): void{
        $this->server->broadcastPackets([$target] ?? [], [
