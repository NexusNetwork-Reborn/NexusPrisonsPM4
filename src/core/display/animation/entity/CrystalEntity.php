...
class CrystalEntity extends Entity implements AnimationEntity {
     ...
     */
    public static function create(Position $position): CrystalEntity {
        $entity = new CrystalEntity(Location::fromObject($position, $position->getWorld()));
        $skin = Nexus::getInstance()->getDisplayManager()->getAnimationManager()->getSkin(ItemIds::DIAMOND_ORE);
        if($skin === null) {
            throw new AnimationException("Unable to find the item skin of id \"-4\"");
        }
        $entity->setSkin($skin);
    ...
    protected function entityBaseTick(int $tickDiff = 1): bool {
        ...
        elseif($y * $y < 0.6 and $this->motion->x === 0 and $this->motion->z === 0) {
            ...
            $this->motion->z = $this->speed * 0.15 * ($z / (abs($x) + abs($z)));
            $this->getWorld()->addParticle($this->pos, new LavaParticle());
        }
...
//        $pk->gameMode = 0;
//        $pk->abilitiesPacket = UpdateAbilitiesPacket::create(0, 0, $this->getId(), []);
