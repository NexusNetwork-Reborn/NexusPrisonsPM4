...
class DiamondOre extends \pocketmine\block\DiamondOre implements Ore {
    ...
    public function __construct(BlockIdentifier $idInfo, string $name) {
        parent::__construct($idInfo, $name, new BlockBreakInfo(3.0, BlockToolType::PICKAXE, ToolTier::IRON()->getHarvestLevel()));
    }
    ...
    public function getDropsForCompatibleTool(Item $item): array {
        ...
        if(mt_rand(1, $chance) == 1) {
            $drops[] = ItemFactory::getInstance()->get(ItemIds::DIAMOND, 0, 1);
        }
        else {
            $drops[] = ItemFactory::getInstance()->get(ItemIds::DIAMOND_ORE, 0, 1);
        }
        return $drops;
    }
    ...
    public function onBreak(Item $item, Player $player = null): bool {
        ...
        if($this->getBreakInfo()->isToolCompatible($item)) {
            ...
            Nexus::getInstance()->getScheduler()->scheduleDelayedTask(new BlockRegenerateTask($this, $this->getPosition()), 500);
            $this->getPosition()->getWorld()->setBlock($this->getPosition(), VanillaBlocks::BEDROCK(), true);
            return true;
        }
        return false;
