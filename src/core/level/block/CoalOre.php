...
class CoalOre extends \pocketmine\block\CoalOre implements Ore {
    ...
    public function __construct(BlockIdentifier $idInfo, string $name) {
        parent::__construct($idInfo, $name, new BlockBreakInfo(2.0, BlockToolType::PICKAXE, ToolTier::WOOD()->getHarvestLevel()));
    }
    ...
    public function getDropsForCompatibleTool(Item $item): array {
        ...
        if(mt_rand(1, $chance) == 1) {
            $drops[] = ItemFactory::getInstance()->get(ItemIds::COAL, 0, 1);
        }
        else {
            $drops[] = ItemFactory::getInstance()->get(ItemIds::COAL_ORE, 0, 1);
        }
        return $drops;
    }
    ...
    public function onBreak(Item $item, Player $player = null): bool {
        ...
        if($this->getBreakInfo()->isToolCompatible($item)) {
            ...
            Nexus::getInstance()->getScheduler()->scheduleDelayedTask(new BlockRegenerateTask($this, $this->getPosition()), 100);
            $this->getPosition()->getWorld()->setBlock($this->getPosition(), VanillaBlocks::BEDROCK(), true);
            return true;
        }
        return false;
