...
class RedstoneOre extends \pocketmine\block\RedstoneOre implements Ore {

    /**
     * RedstoneOre constructor.
     *
     * @param BlockIdentifierFlattened $idInfo
     * @param string $name
     * @param BlockBreakInfo|null $breakInfo
     */
    public function __construct(BlockIdentifierFlattened $idInfo, string $name, ?BlockBreakInfo $breakInfo = null){
        parent::__construct($idInfo, $name, $breakInfo ?? new BlockBreakInfo(3.0, BlockToolType::PICKAXE, ToolTier::GOLD()->getHarvestLevel()));
    }
    ...
    public function getDropsForCompatibleTool(Item $item): array {
        ...
        if(mt_rand(1, $chance) == 1) {
            $drops[] = ItemFactory::getInstance()->get(ItemIds::REDSTONE_DUST, 0, 1);
        }
        else {
            $drops[] = ItemFactory::getInstance()->get(ItemIds::REDSTONE_ORE, 0, 1);
        }
        return $drops;
    }
    ...
    public function onBreak(Item $item, Player $player = null): bool {
        ...
        if($this->getBreakInfo()->isToolCompatible($item)) {
            ...
            Nexus::getInstance()->getScheduler()->scheduleDelayedTask(new BlockRegenerateTask($this, $this->getPosition()), 300);
            $this->getPosition()->getWorld()->setBlock($this->getPosition(), VanillaBlocks::BEDROCK(), true);
            return true;
        }
        return false;
