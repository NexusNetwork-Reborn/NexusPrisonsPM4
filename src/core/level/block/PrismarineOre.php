...
class PrismarineOre extends \pocketmine\block\EmeraldOre implements Ore {
    ...
    public function getDropsForCompatibleTool(Item $item): array {
        ...
        if(mt_rand(1, $chance) == 1) {
            $drops[] = ItemFactory::getInstance()->get(ItemIds::PRISMARINE, 0, 1);
        }
        else {
            $drops[] = ItemFactory::getInstance()->get(ItemIds::PRISMARINE, 0, 1);
        }
    ...
    public function onBreak(Item $item, Player $player = null): bool {
        ...
        // TODO: Chance of Key Component B
        if($this->getBreakInfo()->isToolCompatible($item)) {
            ...
            if(mt_rand(1, 100) === 1) {
                $key = (new KeyComponentB())->toItem();
                if($player->getInventory()->canAddItem($key)) {
                    $player->getInventory()->addItem($key);
                } else {
                    $player->getWorld()->dropItem($this->getPosition(), $key);
                }
            }
